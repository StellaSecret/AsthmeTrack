...
// caught/missed/unviable format as the reference mutants.yml.
//
...
const outDir = args[1];
let excludeLines = new Set();
const optIdx = args.indexOf('--exclude-lines');
if (optIdx !== -1 && args[optIdx + 1]) {
  for (const n of args[optIdx + 1].split(',')) {
    const v = parseInt(n, 10);
    if (!isNaN(v)) excludeLines.add(v);
  }
}

if (!jsonPath || !outDir) {
  console.error('usage: node scripts/mutants-txt.mjs <mutation.json> <outDir> [--exclude-lines <n,n,...>]');
  process.exit(2);
}
...
for (const [file, fileResult] of Object.entries(files)) {
  for (const mutant of fileResult.mutants || []) {
    ...
    const lineNo = mutant.location?.start?.line;
    if (excludeLines.has(lineNo)) {
      // Mirror of cargo-mutants --exclude-lines: these mutants are not scored.
      console.log(`Excluded (--exclude-lines ${lineNo}): ${ref}`);
      continue;
    }
    total += 1;
