...
test.describe('Saisie d\'une mesure complète', () => {
  ...
  test('enregistrer une mesure valide redirige vers le dashboard', async ({ page }) => {
    ...
    await addMeasure(page, {
      datetime: '2025-01-15T08:00',
      dep1: 380, dep2: 390, dep3: 400,
      spo2: 97,
      easy: 1,
    });

    // Après enregistrement, l'app revient au dashboard
  ...
  test('SpO2 manquante bloque l\'enregistrement', async ({ page }) => {
    ...
    await page.locator('#dep1').fill('380');
    await page.locator('.easyh-btn[data-val="1"]').click();
    // On ne remplit pas SpO2
    await page.locator('button.btn-primary', { hasText: /enregistrer/i }).click();

    await expect(page.locator('#toast.show')).toBeVisible();
    await expect(page.locator('#page-saisie')).toHaveClass(/active/); // on ne quitte pas la page
  });
...
test.describe('Historique', () => {
  ...
  test('modifier une mesure met à jour les données', async ({ page }) => {
    ...
    await page.locator('.history-item .btn-secondary', { hasText: /modifier/i }).first().click();
    ...
    await page.locator('#editModal .btn-primary', { hasText: /enregistrer/i }).click();
  ...
  test('supprimer une mesure la retire de l\'historique', async ({ page }) => {
    await seedMeasures(page, [
      fakeMeasure({ id: 1, dep: 380 }),
      fakeMeasure({ id: 2, dep: 410 }),
    ]);
    await seedBestDEP(page, 450);
    await page.goto('/');
    await goToTab(page, 'historique');
    ...
    await page.locator('.history-item .btn-secondary', { hasText: /supprimer/i }).first().click();
...
test.describe('Dashboard — métriques', () => {
  ...
  test('affiche le DEP de la dernière mesure', async ({ page }) => {
    await seedMeasures(page, [
      ...
      fakeMeasure({ id: 2, dt: '2025-01-15T08:00', dep: 440 }), // plus récente
    ]);
    await seedBestDEP(page, 450);
    await page.goto('/');

    // La mesure la plus récente (440) doit apparaître dans les métriques
  ...
  test('la zone DEP verte s\'affiche pour un DEP >= 80% du meilleur', async ({ page }) => {
    // bestDEP = 400, DEP = 340 → 85% → zone verte
  ...
  test('la zone DEP rouge s\'affiche pour un DEP < 60% du meilleur', async ({ page }) => {
    // bestDEP = 500, DEP = 250 → 50% → zone rouge
