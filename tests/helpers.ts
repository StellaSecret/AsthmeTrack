 ...
 *  L'app stocke les mesures avec unshift (plus récente en index 0).
 *  Passe les mesures dans l'ordre que tu veux — le helper les trie newest-first. */
export async function seedMeasures(page: Page, measures: object[]) {
  await page.addInitScript((data) => {
    // Trie newest-first pour correspondre à l'ordre de DB.measures (unshift)
 ...
 *  Tous les sélecteurs sont scoped à #page-saisie pour éviter les conflits
 *  avec le modal d'édition qui est présent dans le DOM en même temps. */
export async function addMeasure(page: Page, opts: {
  ...
  if (opts.datetime) {
    await form.locator('#inputDatetime').fill(opts.datetime);
  }
  ...
  if (opts.comment) {
    await form.locator('#inputComment').fill(opts.comment);
  }
